...
def build_reusable_value_map(env, task_description, stage:int = 0, object_path = "/home/zijianwang/openpi/third_party/LIBERO-PRO/reordered_objects_with_descriptions.json"):
    """
...
def generate_candidate_action_chunks(client, element, replan_steps):
    """
    生成候选动作序列。
    ...
    Returns:
        np.ndarray: 候选动作序列 (shape: [batch_size, horizon, action_dim])
    """
    ...
    # Ensure 3D shape: [batch_size, horizon, action_dim]
...
def select_best_action_chunk_using_valuemap(candidate_action_chunks, env, reusable_valuemap, replan_steps, logger, t, current_episode_costs):
    """
    从候选动作序列中选择最佳动作序列。
    
    Args:
        candidate_action_chunks: 候选动作序列 (shape: [batch_size, horizon, action_dim])
        env: LIBERO 环境实例
    ...
    try:
        batch_eef_positions = []
        for i in range(candidate_action_chunks.shape[0]):
            eef_traj = compute_eef_trajectory_from_actions(env, candidate_action_chunks[i])
        ...
        if reusable_valuemap is not None:
            ...
            best_action_chunk = candidate_action_chunks[best_traj_id]
            ...
            best_action_chunk = candidate_action_chunks[0]
...
def select_best_action_chunk_using_angles(candidate_action_chunks, angles):
    """
    从候选动作序列中选择angle最大的动作序列。
    
    Args:
        candidate_action_chunks: 候选动作序列 (shape: [batch_size, horizon, action_dim])
        angles: angle值列表，每个元素对应一个候选动作序列
        
    Returns:
        Tuple[np.ndarray, float]: angle最大的动作序列 (shape: [horizon, action_dim]) 和对应的最大角度值
    """
    if len(angles) == 0:
        raise ValueError("angles list is empty")
    
    if len(angles) != candidate_action_chunks.shape[0]:
        raise ValueError(f"Length mismatch: angles has {len(angles)} elements, but candidate_action_chunks has {candidate_action_chunks.shape[0]} elements")
    
    # 找到angle最大的索引
    best_idx = np.argmax(angles)
    best_action_chunk = candidate_action_chunks[best_idx]
    max_angle = round(angles[best_idx], 3)
    
    return best_action_chunk, max_angle
...
def search_best_action_chunk(client, element, env, reusable_valuemap, replan_steps, logger, t, current_episode_costs):
    ...
    return best_action_chunk, action_chunk